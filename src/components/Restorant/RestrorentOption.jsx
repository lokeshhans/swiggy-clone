...
const RestrorentOption = () => {
  ...
  const swiggyAPI =
    "https://www.swiggy.com/mapi/restaurants/list/v5?offset=0&is-seo-homepage-enabled=true&lat=28.8964&lng=76.5909";
  useEffect(() => {
    async function fetchData() {
      const RestData = await axios.get(proxyServer + swiggyAPI);
      const data = await RestData?.data?.data?.cards[1]?.card?.card
      ...
      <div className="w-full  flex flex-col items-center pt-20  bg-white">
        ...
        <div className="px-auto  w-[80%] h-30vh    ">
          <div className="flex flex-nowrap overflow-auto gap-4">
            {ResData.map((item) => (
              <RestorandCard key={item.id} item={item} />
            ))}
          </div>
        </div>
      </div>
